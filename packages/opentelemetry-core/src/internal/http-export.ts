...
import { createFetchRequest, isFetchRequestAvailable } from './http-clients/fetch';
...
const clients = {
  fetch: [isFetchRequestAvailable, createFetchRequest],
  'node:http': [internal.isHttpRequestAvailable, internal.createHttpRequest],
 ...
 */
export class HttpExportClient {
  private _request: RequestFunction;
  constructor(preferredClients: HttpClient[]) {
    this._request = determineClient(preferredClients);
  }
  ...
  export(
    ...
    const errorHandler = (
      ...
      return setTimeoutPromise(err.retryAfterMillis)
        .then(() => this._request(url, payload, options))
        .then(okHandler, errorHandler);
    };
    const p = this._request(url, payload, options).then(okHandler, errorHandler);
